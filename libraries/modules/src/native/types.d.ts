export namespace DiscordNativeModules {
    ...
    export interface ClientInfoModule {
        ...
        ReleaseChannel: string | 'canaryRelease' | 'googleRelease'
        /**
         * Matches `Version`
         */
        OTABuild: string
    ...
    /**
     * Manage OTA updates, app reloads, etc.
     */
     ...
     *
     * This only manages the settings native-side. Already rendered native components will need to be re-rendered to apply changes.
     * Native components that react to these changes are really random, here are the document ones:
     * - Message list components
     *   - Guild tags (the chip background does not change, only the text color)
     *   - Reply previews
     *   - Mentions
     *   - Codeblocks
     *   - Spoilers
     *   - Attachment backgrounds (when they have not fully loaded)
     *   - Embeds (the background color does not change, only the content)
     *   - Reactions
     * - Status bar color
     */
    export interface ThemeModule {
        /**
         * Sets the saturation value (1 is default)
